...
font = pygame.font.SysFont(None, 20)  # Use default font to avoid 'arial' issues
...
running = True
# Button and line state
button_rect = pygame.Rect(20, 20, 100, 40)  # Button at top-left
line_state = None  # None, "positive", or "negative"
line_p1 = None
line_q1 = None
...
while running:
    for event in pygame.event.get():
        ...
        if event.type == pygame.MOUSEBUTTONDOWN:
            if button_rect.collidepoint(event.pos):
                # Toggle line orientation randomly
                line_state = random.choice(["positive", "negative"])
                # Set endpoints based on orientation
                if line_state == "positive":
                    start_r = 4
                    start_c = 0
                    end_r = 7
                    end_c = 3
                    ...
                    end_r = 7
                    end_c = 4
                start_index = start_r * (start_r + 1) // 2 + start_c
                end_index = end_r * (end_r + 1) // 2 + end_c
                line_p1 = pegs[start_index]
                line_q1 = pegs[end_index]
    ...
    button_text = font.render("Draw Line", True, WHITE)
    button_text_rect = button_text.get_rect(center=button_rect.center)
    screen.blit(button_text, button_text_rect)

    # Draw line if active
    if line_state:
        pygame.draw.line(screen, WHITE, line_p1, line_q1, 2)
